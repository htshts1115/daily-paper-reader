...
tags: ["query:matting"]
score: 8.0
evidence: 利用视差的双眼人像抠图，估计trimap与前景背景运动
tldr: 单图抠图在前景与背景纹理丰富时高度病态，现有方法依赖绿幕或偏振光等特殊采集条件。本文提出视差人像抠图，利用连拍中轻微视角变化带来的前景背景视差作为补充观测，先估计trimap与前景背景运动，再据此构建抠图流程。该方法在自然拍摄条件下提升人像抠图质量，为手持连拍场景下的高质量人像抠图提供了实用新途径。
...
motivation: 单图抠图在纹理复杂场景下病态，现有方法依赖绿幕、偏振光或干净背景等特殊采集设置，难以普适。
method: 提出双帧视差人像抠图，利用连拍中轻微视角变化产生的视差，先估计trimap与前景背景运动再构建抠图流程。
result: 该方法在自然连拍条件下有效提升人像抠图质量，尤其改善纹理丰富场景的前景背景分离。
conclusion: 视差作为自然可获得线索，为实用人像抠图提供了无需专用硬件的新方向。
---
